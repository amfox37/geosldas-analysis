...
At each tile, the calendar-month climatology was removed using a trend-preserving seasonal adjustment, and the long-term change was estimated from the exact Theil–Sen median slope. Trend significance was assessed using a modified Mann–Kendall test that accounts for serial correlation, followed by Benjamini–Hochberg false-discovery-rate control at 0.05 over each complete spatial field. OL, DA, and paired DA−OL trends were evaluated on the same underlying samples but retained separate spatial FDR families. Area-weighted domain-mean trends were evaluated separately from the tile-scale fields. Mapped significance is based on the FDR-controlled test rather than on pointwise confidence intervals.

Potential observing-system discontinuities were then assessed using the P1–P9 chronology defined in Fig. 1. This analysis used area-weighted monthly domain-mean series and was deliberately separated from the whole-record trend analysis because a discrete level change or change in slope need not imply a secular trend over the complete record. Known observing-system boundaries were evaluated with a segmented interrupted time-series model containing the seasonal cycle, level changes at each P2–P9 boundary, and slope changes where the duration of the adjacent periods permitted them. Serial dependence was represented using iterative Prais–Winsten AR(1) fitting, and uncertainty in the transition coefficients was estimated from fitted-AR(1) bootstrap realizations. False-discovery-rate control was applied separately within each observing-system boundary family. Because P7 spans only 15 months, it was tested for a level change but was not assigned an independently estimated period slope.

As an independent check on the prescribed observing-system dates, abrupt changepoints were also identified without supplying the P1–P9 boundaries to the detector. Two Pruned Exact Linear Time (PELT) formulations were applied to trend-preserving, seasonally adjusted monthly series across a predeclared range of penalties. A changepoint was accepted only if it was retained by the primary piecewise AR(1)-plus-linear formulation, remained stable across the penalty range, and was reproduced within three months by a separately prewhitened linear formulation. Accepted changepoints were subsequently compared with the known P2–P9 dates using ±3 months as the primary matching tolerance and ±6 months as a sensitivity. Unmatched changepoints were retained rather than assigned to the nearest observing-system event. This conservative procedure is most sensitive to abrupt structural changes and has limited ability to localize gradual slope transitions; consequently, the known-date interrupted-series analysis remains the primary test of slope changes.

Known-date and independently detected changes were interpreted together. Agreement between a detected discontinuity and an observing-system boundary strengthens the evidence for a change in analysis regime, but temporal correspondence alone is not treated as proof that the newly introduced observing system caused every contemporaneous change.
