#!/usr/bin/env python3
"""
Step 1.5 v2 of the revised CYGNSS L1 single-observation diagnosis: re-run the
Kalman-gain-consistency check (see extract_cygl1_gain_consistency.py) against
the RE-THINNED (window-alignment-bug-fixed) single-obs-per-window experiment
(DAv8_M36_all_sensors_AZ_scaled_cygl1assim_singleobs_v2, job 58019976), where
each assimilated CYGNSS L1 obs is genuinely isolated (>5deg from every other
kept obs in the same TRUE centered assimilation window -- outside its own and
every other kept obs's 2.5deg Gaspari-Cohn compact-support ellipse). Unlike
the original Step 1 (full obs stream, ~56 neighbors/obs, confound invalidated
the K*d identity), this run has 0 confounding neighbors by construction, so:
...
CSV_PATH = os.path.join(OUT_DIR, "cygl1_singleobs_v2_gain_consistency_ofa.csv.gz")
SUMMARY_PATH = os.path.join(OUT_DIR, "cygl1_singleobs_v2_gain_consistency_summary.txt")
...
def main():
    ...
    summary_lines = []
    summary_lines.append("CYGNSS L1 gain-consistency check, Step 1.5 v2 (re-thinned, window-bug-fixed, job 58019976)")
    ...
    header_lines = ["# CYGNSS L1 gain-consistency check, Step 1.5 v2 (re-thinned, window-bug-fixed)"]
    ...
    df.to_pickle(os.path.join(OUT_DIR, "_cygl1_singleobs_v2_gain_consistency.pkl"))
    return df
