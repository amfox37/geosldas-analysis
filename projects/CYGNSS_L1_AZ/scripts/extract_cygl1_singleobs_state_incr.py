#!/usr/bin/env python3
"""
Follow-up to extract_cygl1_singleobs_gain_consistency.py: for each of the 47
genuinely-isolated CYGNSS L1 obs (Step 1.5, DAv8_M36_all_sensors_AZ_scaled_
cygl1assim_singleobs, job 58019133), join the same-cycle catch_progn_incr
collection at the owner tile to see the actual state-space increment split.
...
GAIN_CSV = os.path.join(OUT_DIR, "cygl1_singleobs_gain_consistency_ofa.csv.gz")
OUT_CSV = os.path.join(OUT_DIR, "cygl1_singleobs_state_incr.csv.gz")
SUMMARY_PATH = os.path.join(OUT_DIR, "cygl1_singleobs_state_incr_summary.txt")
...
def main():
    ...
    summary_lines = []
    summary_lines.append("CYGNSS L1 state-space increment check, Step 1.5 follow-up")
    ...
    with gzip.open(OUT_CSV, "wt") as fo:
        fo.write("# CYGNSS L1 state-space increment check, Step 1.5 follow-up\n")
