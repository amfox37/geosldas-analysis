#!/usr/bin/env python3
"""
CYGNSS L1 paired thinning-density experiment (2026-08-22): extract assimilated
(assim_flag==1) CYGNSS_L1_DDM3X5_CROP_SCALAR ObsFcstAna (OFA) rows from the
three DA arms of the 4-arm paired density experiment (jobs 58020767-70,
20200101-20201101, monthly self-resubmitting, all completed cleanly):

    DA-sparse:       DAv8_M36_AZ_paired_cygl1_sparse        (min_sep=5.0deg, isolated)
    DA-intermediate: DAv8_M36_AZ_paired_cygl1_intermediate  (min_sep=2.40deg, nested superset of sparse)
    DA-dense:        DAv8_M36_AZ_paired_cygl1_dense          (full unthinned L1 stream)

Generalizes extract_cygl1_singleobs_v2_gain_consistency.py to loop over the
3 arms (adds an "arm" column) and restricts the OFA glob to Y2020/M05-M10
(May-Oct 2020 evaluation window; Jan-Apr is spin-up, excluded entirely, files
not even read). Includes tilenum (not just tile_id) in the output -- needed
for the tile-membership join used by the local-interaction-count covariate
(see cygl1_paired_local_interaction_join.py).
...
CSV_PATH = os.path.join(OUT_DIR, "cygl1_paired_gain_consistency_ofa.csv.gz")
SUMMARY_PATH = os.path.join(OUT_DIR, "cygl1_paired_gain_consistency_summary.txt")
...
def main():
    ...
    summary_lines = []
    summary_lines.append("CYGNSS L1 paired thinning-density experiment: OFA extraction, 3 DA arms, May-Oct 2020")
    ...
    header_lines = ["# CYGNSS L1 paired thinning-density experiment: OFA extraction, 3 DA arms, May-Oct 2020"]
    ...
    header_lines.append("# months restricted to Y2020 M05-M10 (evaluation window; Jan-Apr spin-up excluded, not read)")
    ...
    df.to_pickle(os.path.join(OUT_DIR, "_cygl1_paired_gain_consistency.pkl"))
    return df
