#!/usr/bin/env python3
"""
22-month event-level OFA extraction for DAv8_M36_AZ_paired_cygl1_dense_gated
(full/unthinned CYGNSS L1 stream, hard-gated binary commit 406206a). Extends
the earlier inline (heredoc, unsaved) Jan/Jan+Feb/full-year-2020 diagnostics
to the full Jan2020-Dec2021 record now that the 12-month extension completed
(cap_restart=20220101, exit 0:0 throughout).

Full calendar-year period both years, no spin-up exclusion (matches this
arm's own convention -- it's the gated-dense benchmark itself, not a
May-Oct-evaluation-window arm like the sparse/intermediate/dense family).

Same extraction logic as extract_cygl1_paired_gain_consistency_22mo.py, just
one arm (gated_dense) and the wider Jan2020-Dec2021 month range.
"""
import glob
import os
import sys
...
ARMS = {
    "gated_dense": "DAv8_M36_AZ_paired_cygl1_dense_gated",
}
EXP_ROOT = "/discover/nobackup/projects/land_da/cygl1_operator_test"

# Jan 2020 - Dec 2021, full calendar years (no spin-up exclusion)
YEAR_MONTHS_WANTED = [(2020, m) for m in range(1, 13)] + [(2021, m) for m in range(1, 13)]

OUT_DIR = "/gpfsm/dnb06/projects/p284/geosldas-analysis/projects/CYGNSS_L1_AZ/output"
CSV_PATH = os.path.join(OUT_DIR, "cygl1_gated_dense_gain_consistency_22mo_ofa.csv.gz")
SUMMARY_PATH = os.path.join(OUT_DIR, "cygl1_gated_dense_gain_consistency_22mo_summary.txt")
...
def extract_arm(arm, exp_id):
    ...
    files = sorted(files)
    print(f"[{arm}] Found {len(files)} OFA files (Jan2020-Dec2021)", flush=True)
    if len(files) == 0:
...
def main():
    ...
    summary_lines = []
    summary_lines.append("CYGNSS L1 gated-dense arm: OFA extraction, Jan2020-Dec2021 (22-month extension)")
    ...
    header_lines = ["# CYGNSS L1 gated-dense arm: OFA extraction, Jan2020-Dec2021 (22-month extension)"]
    ...
    header_lines.append("# months: Jan2020-Dec2021, full calendar years (no spin-up exclusion)")
    ...
    df.to_pickle(os.path.join(OUT_DIR, "_cygl1_gated_dense_gain_consistency_22mo.pkl"))
    return df
