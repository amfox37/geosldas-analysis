#!/usr/bin/env python3
"""
CYGNSS L1 paired thinning-density + coherency-screening (coh05) follow-up,
12-month extension: DAv8_M36_AZ_paired_cygl1_intermediate_coh05 and
DAv8_M36_AZ_paired_cygl1_dense075_coh05 were extended from END_DATE 20200701
(6mo) to END_DATE 20210101 (jobs 58270731/58270732, submitted+completed
2026-09-07; dense_coh05 was NOT extended). This compares full Jan-Dec 2020
stats for these two arms against their own per-arm OL cross-mask baseline
(OL_paired_monitor evaluated on that exact arm's obs population, via
use_obs=True on the DA arm when building the sums -- see
run_cygl1_paired_OL_xmask_coh05_12mo.py), which is the correct "does DA beat
OL" comparison for species 13 (CYGNSS L1) per feedback_da_evaluation_standard,
and equivalent to the plain OL baseline for species 1-12 (monitor obs
populations coincide for OL and all DA arms).
...
Inputs: spatial_stats_{DA,OL_paired_monitor_xmask}_{arm}_202001_202012.pkl,
built by run_cygl1_paired_density_coh05_12mo.py /
run_cygl1_paired_OL_xmask_coh05_12mo.py (postproc_ObsFcstAna toolkit).
"""
import pickle
import numpy as np
...
def main():
    print("=" * 100)
    print("Jan-Dec 2020 (full 12-month extension), Arizona-box paired experiment:")
    ...
    for arm in ARMS:
        da_sp, da_grp = load(f"spatial_stats_DA_paired_{arm}_202001_202012.pkl")
        ol_sp, ol_grp = load(f"spatial_stats_OL_paired_monitor_xmask_{arm}_202001_202012.pkl")
    ...
    for arm in ARMS:
        da_sp, _ = load(f"spatial_stats_DA_paired_{arm}_202001_202012.pkl")
        ol_sp, _ = load(f"spatial_stats_OL_paired_monitor_xmask_{arm}_202001_202012.pkl")
        print(f"\n-- {arm} --")
