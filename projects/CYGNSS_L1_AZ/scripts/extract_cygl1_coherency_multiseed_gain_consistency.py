#!/usr/bin/env python3
"""
CYGNSS L1 coherency-screening experiment, multi-seed re-draw: extract
assimilated (assim_flag==1) CYGNSS_L1_DDM3X5_CROP_SCALAR ObsFcstAna (OFA)
rows for ALL FOUR arms:

    coherency_screened (Arm A):      DAv8_M36_AZ_paired_cygl1_coherency_screened
    coherency_randmatch (Arm B seed1, 20260827): DAv8_M36_AZ_paired_cygl1_coherency_randmatch
    coherency_randmatch_seed2 (Arm B seed2, 20260828): DAv8_M36_AZ_paired_cygl1_coherency_randmatch_seed2
    coherency_randmatch_seed3 (Arm B seed3, 20260829): DAv8_M36_AZ_paired_cygl1_coherency_randmatch_seed3

Purpose: check whether Arm A's ~2x CygL1-skill advantage over the original
single-seed Arm B (extract_cygl1_coherency_gain_consistency.py) is a real
effect or a single-seed artifact, by adding two more independent Arm-B draws.

Does NOT touch extract_cygl1_coherency_gain_consistency.py or its outputs --
this is a superset extraction (same logic, all 4 arms) writing to new files.
...
ARMS = {
    ...
    "coherency_randmatch_seed2": "DAv8_M36_AZ_paired_cygl1_coherency_randmatch_seed2",
    "coherency_randmatch_seed3": "DAv8_M36_AZ_paired_cygl1_coherency_randmatch_seed3",
}
...
CSV_PATH = os.path.join(OUT_DIR, "cygl1_coherency_multiseed_gain_consistency_ofa.csv.gz")
SUMMARY_PATH = os.path.join(OUT_DIR, "cygl1_coherency_multiseed_gain_consistency_summary.txt")
...
def main():
    ...
    summary_lines = []
    summary_lines.append("CYGNSS L1 coherency-screening experiment: multi-seed OFA extraction, 4 arms, Jan-Dec 2020")
    summary_lines.append(f"species_name = {SPECIES_NAME_WANTED}")
    summary_lines.append("")
    summary_lines.append(f"{'arm':28s} {'species_id':>10s} {'N':>8s}")
    for arm in ARMS:
        n = int((df["arm"] == arm).sum())
        summary_lines.append(f"{arm:28s} {species_ids[arm]:>10d} {n:>8d}")
    summary_text = "\n".join(summary_lines)
    print(summary_text)

    header_lines = ["# CYGNSS L1 coherency-screening experiment: multi-seed OFA extraction, 4 arms, Jan-Dec 2020"]
    ...
    df.to_pickle(os.path.join(OUT_DIR, "_cygl1_coherency_multiseed_gain_consistency.pkl"))
    return df
