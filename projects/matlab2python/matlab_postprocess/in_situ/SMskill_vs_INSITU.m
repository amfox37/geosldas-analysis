...
INSITU_tag ='USCRN'; % 'USCRN', 'Oznet', 'Msnet' , 'SMOSM', 'CalVal_M33', 'SCAN??'
...
exp_run_name= {...
        'DAv7_M36_ASCAT_type_2'; ...
        'DAv7_M36'; ...
        'smap_ascat_multivariate_test'; ...
        'OLv7_M36'; ...
        'DAv7_M36_ASCAT_type_2_le'; ...
        'OLv7_M36_ascat_fp_precip'; ...
        'DAv7_M36_ASCAT_type_2_fp_precip' ...
     };

% tmp mat file to store the time series
fout_name = [fout_path,'/Expt_name_',INSITU_tag, '_SM_3h_',rzmc_tag,'_6yr_']
...
if exist([fout_name,'raw_timeseries.mat'],'file')
    ...
    exp_path = { ...
        '/discover/nobackup/amfox/Experiments/DAv7_M36_ASCAT_type_2'; ...
        '/home/amfox/smap/SMAP_Nature/SMAP_Nature_v10'; ...
        '/discover/nobackup/amfox/Experiments/smap_ascat_multivariate_test'; ...
        '/home/amfox/smap/SMAP_Nature/SMAP_Nature_v10'; ...
        '/discover/nobackup/amfox/Experiments/DAv7_M36_ASCAT_type_2_le'; ...
        '/discover/nobackup/amfox/Experiments/OLv7_M36_ascat_fp_precip'; ...
        '/discover/nobackup/amfox/Experiments/DAv7_M36_ASCAT_type_2_fp_precip' ...
        };
    ...
    fname_tilecoord = [ exp_path{end}, '/', exp_run{end}, '/output/', domain{end}, '/rc_out/', ...
        exp_run{end}, '.ldas_tilecoord.bin'];
    ...
    for kk = 1:length(exp_run)
        
        % re-read tilecoord file for each run in case the tile orders are different
        
        fname_tilecoord = [ exp_path{kk},'/', exp_run{kk}, '/rc_out/',exp_run{kk}, '.ldas_tilecoord.bin'];
        
        if ~exist(fname_tilecoord,'file')
            fname_tilecoord = [ exp_path{kk}, '/', exp_run{kk}, '/output/', domain{kk}, '/rc_out/', ...
                exp_run{kk}, '.ldas_tilecoord.bin'];
        end
        ...
        while 1
            ...
            fname = [exp_path{kk}, exp_run{kk}, '/output/', domain{kk},          ...
                    '/cat/ens_avg',                                       ...
                    '/Y',   num2str(date_time.year, '%4.4d'),         ...
                    '/M',   num2str(date_time.month,'%2.2d'),         ...
                    '/', exp_run{kk}, out_collection,                          ...
            ...
            LDAS_sm_org(t_ind,1,:,kk) = tile_data_tmp(ind_tile,ind_hour);  %sfmc
            clear tile_data_tmp
            tile_data_tmp = ncread(fname, 'sm_rootzone');
            LDAS_sm_org(t_ind,2,:,kk) = tile_data_tmp(ind_tile,ind_hour); 
            clear tile_data_tmp

            tile_data_tmp = ncread(fname, 'surface_temp');
            LDAS_st_org(t_ind,1,:,kk) = tile_data_tmp(ind_tile,ind_hour);
            clear tile_data_tmp

            tile_data_tmp = ncread(fname, 'soil_temp_layer1');
            LDAS_st_org(t_ind,2,:,kk) = tile_data_tmp(ind_tile,ind_hour);
            clear tile_data_tmp

            tile_data_tmp = ncread(fname,'precipitation_total_surface_flux');
            LDAS_prcp_org(t_ind,1,:,kk) = tile_data_tmp(ind_tile,ind_hour); 
            clear tile_data_tmp
...
for kk = 1:length(exp_run)
    ...
    LDAS_sm(:,1,:) = LDAS_sm_org(:,1,:,kk);
    LDAS_sm(:,2,:) = LDAS_sm_org(:,2,:,kk);  %rzmc
    ...
    for i = 1:size(LDAS_sm,3)
        for j = 1:size(LDAS_sm,2)
            ...
            R(i,j,kk)   = stats(strcmp(stats_tags,'R'));
            RLO(i,j,kk) = stats(strcmp(stats_tags,'RLO')) - R(i,j,kk) ;
            RUP(i,j,kk) = stats(strcmp(stats_tags,'RUP')) - R(i,j,kk);
            
            Bias(i,j,kk)   = -stats(strcmp(stats_tags,'bias'));
            BiasLO(i,j,kk) = -stats(strcmp(stats_tags,'CI_bias')) ;
            BiasUP(i,j,kk) =  stats(strcmp(stats_tags,'CI_bias')) ;
            
            absBias(i,j,kk) =  abs(stats(strcmp(stats_tags,'bias')));
            absBiasLO(i,j,kk) = -stats(strcmp(stats_tags,'CI_bias')) ;
            absBiasUP(i,j,kk) =  stats(strcmp(stats_tags,'CI_bias')) ;
            
            RMSE(i,j,kk) =     sqrt( stats(strcmp(stats_tags,'MSE')) );
            RMSELO(i,j,kk) = -sqrt(stats(strcmp(stats_tags,'CI_MSE'))) ;
            RMSEUP(i,j,kk) =  sqrt(stats(strcmp(stats_tags,'CI_MSE'))) ;
            ubRMSE(i,j,kk) =   sqrt(stats(strcmp(stats_tags,'MSE')) - stats(strcmp(stats_tags,'bias')).^2);
            ubRMSELO(i,j,kk) = -sqrt(stats(strcmp(stats_tags,'CI_MSE'))+stats(strcmp(stats_tags,'MSE')) - ...
                stats(strcmp(stats_tags,'bias')).^2)+ubRMSE(i,j,kk);
            ubRMSEUP(i,j,kk) =  sqrt(stats(strcmp(stats_tags,'CI_MSE'))+stats(strcmp(stats_tags,'MSE')) - ...
                stats(strcmp(stats_tags,'bias')).^2)-ubRMSE(i,j,kk);
            ...
            if add_anomR
                ...
                anomR(i,j,kk)   = stats(strcmp(stats_tags,'R'));
                anomRLO(i,j,kk) = stats(strcmp(stats_tags,'RLO')) - anomR(i,j,kk) ;
                anomRUP(i,j,kk) = stats(strcmp(stats_tags,'RUP')) - anomR(i,j,kk);
