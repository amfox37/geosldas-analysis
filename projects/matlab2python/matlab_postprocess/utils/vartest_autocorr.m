function [h,p,ci,stats] = vartest_autocorr(x,v,varargin)
...
%         defaults = {0.05    'both'  ''};
        okargs =   {'alpha' 'tail' 'dim' 'sample_s'}; %GDL 7feb14
...
if ~isnan(sample_s)
    df = max(sample_s - 1,0);  %GDL, 7Feb14
else
...
%GDL, 13oct15
if ~isnan(sample_s)
    sumsq = sum(abs(xcntr).^2,dim).*sample_s./max(1,samplesize);
end
if v>0
   chisqstat = sumsq ./ v;
else
