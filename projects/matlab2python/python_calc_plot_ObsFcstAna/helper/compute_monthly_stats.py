    ...
    while date_time < end_time:
        ...
        if len(OFA['obs_tilenum']) > 0:
            ...
            for ispec in np.arange(n_spec):
                ...
                print(f"masked_tilenum: {masked_tilenum}, shape: {masked_tilenum.shape}")
                print(f"tc['tile_id']: {tc['tile_id']}, shape: {tc['tile_id'].shape}")
                print(f"tile_idx: {tile_idx}, shape: {tile_idx.shape}")

                for var in var_list:
                    if tile_idx.size > 0:
                        data_tile[var][tile_idx, ispec] = masked_data[var]
                    else:
                        print(f"Warning: No matching tiles found for masked_tilenum: {masked_tilenum}")       
